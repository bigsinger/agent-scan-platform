...
class LocalScanEngine:
    ...
    def run_assessment(self, request: ScanRequest, name: str = "本地 Agent 安全测评") -> ScanResult:
        ...
        events.append(self._event(assessment["id"], "assessment.started", "测评已启动，进入本地只读扫描", {"target": display_target}))
        ...
        if target is not None and not target.exists():
            ...
            events.append(self._event(assessment["id"], "assessment.failed", "目标路径不存在或不可访问", {"target": str(target)}))
