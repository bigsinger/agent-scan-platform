...
| 轻量模式 | `/assessment` 默认使用独立轻量工作台，一键完成本机发现、150 文件只读快检、风险和报告；完整工作台保留在 `/assessment/advanced` |
| 轻量启动 | `start_services.ps1 -Lite` 只启动 8000 主平台，不启动 4318 OTel Receiver；manifest 明确记录 `mode=lite` |
