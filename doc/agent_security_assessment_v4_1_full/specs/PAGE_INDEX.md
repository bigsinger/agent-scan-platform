...
实现说明：D05 在正式应用中使用 `agent-scan-issues` 运行态页面组件，保留 `/assessment/agent-scan/issues` 独立深链。页面读取 `GET /api/v1/agent-scan/issues?page_size=200` 与 `/api/v1/agent-scan/compat`，展示映射统计、Issue Code 到本地规则的字段映射、原始 JSON 和本地桥接状态；映射测试只调用本地 `POST /api/v1/agent-scan/self-test`，不得访问 Snyk 云或启动、修改已安装 Agent。
