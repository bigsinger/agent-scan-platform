...
首次打开新库时，资产、任务、风险、证据、报告和执行进程列表应为空。这是正常行为：系统不会再用原型 seed 伪造 Agent、MCP、任务或风险数据。只有执行“发现本机”“快速扫描”“Skill 扫描”“红队 dry-run”等本地动作后，相关记录才会写入 SQLite 并出现在页面中。旧版本遗留的已知原型 seed 记录会在启动初始化时从本系统 SQLite 中清理，不会改动已安装 Agent。
